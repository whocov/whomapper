...
#' @param include_adm0_line - should the specialised adm0 borders be used? Note that these are borders **excluding** coastlines.
...
who_map_pipeline <- function(sf = whomapper::pull_sfs(adm_level = 0, query_server = FALSE),
  ...
  out <- append(
    ...
    list(who_map_theme(xlim = xlim, ylim = ylim, box_lims = box_lims),
         who_map_annotate(region = region, data_source = data_source, production_team = production_team))
  )
